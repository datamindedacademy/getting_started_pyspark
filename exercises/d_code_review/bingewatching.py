...
def substring_f(startpos, length):
    return sparkfun.udf(lambda column: column[startpos - 1 : startpos - 1 + length])
...
tv_sessions = tv_sessions.filter(
    ...
    (tv_sessions.main_genre == "13.") & (tv_sessions.programGenreLevel != "13.6")
)  # no soaps
...
joined_and_filtered = joined_and_filtered.where(joined_and_filtered.days_spread <= 7)
