...
if __name__ == "__main__":
    ...
    csv_file_path = (
        Path(__file__).parents[1] / "resources" / "pms_under_elizabeth_2nd.csv"
    )
    ...
    frame.select("consecutive_terms").withColumn(
        "foo", psf.col("consecutive_terms") + 10
    ).show()
    ...
    print(
        result[-1][0]
    )  # the minus in -1 means: start from the back. The element at -1 is the last element.
